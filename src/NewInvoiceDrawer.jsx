...
function inputCls(hasError) {
  return `w-full border rounded-md px-4 py-3 text-sm font-bold text-[#0c0e16] outline-none transition-colors ${
    hasError
...
export default function NewInvoiceDrawer({ onClose, onSave }) {
  ...
  const fieldErrors = submitted
    ? Object.keys(rawErrors).filter(
        (k) => !k.startsWith("item_") && k !== "noItems",
      )
    : [];
  const hasItemErrors = submitted
    ? Object.keys(rawErrors).some((k) => k.startsWith("item_"))
    : false;
  ...
  return (
    <>
      ...
      <div className="fixed top-0 left-[72px] h-full w-[620px] bg-white z-50 flex flex-col rounded-r-2xl overflow-hidden shadow-2xl">
        <div className="flex-1 overflow-y-auto px-10 py-10">
          <h2 className="text-2xl font-bold text-[#0c0e16] mb-10">
            New Invoice
          </h2>
          ...
          <div className="grid grid-cols-2 gap-4 mb-5">
            ...
            <div>
              ...
              <div className="relative">
                <select
                  className="w-full border border-[#dfe3fa] rounded-md px-4 py-3 text-sm font-bold text-[#0c0e16] outline-none focus:border-[#7c5dfa] appearance-none pr-10 transition-colors"
          ...
          <button
            onClick={addItem}
            className="w-full py-4 rounded-full bg-[#f9fafe] hover:bg-[#dfe3fa] text-sm font-bold text-[#7e88c3] transition-colors border-none cursor-pointer mb-4"
          >
            + Add New Item
          </button>
        ...
        {/* ── Footer ── */}
        <div className="flex items-center justify-between px-10 py-6 shadow-[0_-4px_16px_rgba(72,84,159,0.1)] bg-white">
          <button
            onClick={onClose}
            className="px-6 py-4 rounded-full text-sm font-bold bg-[#f9fafe] text-[#7e88c3] hover:bg-[#dfe3fa] transition-colors border-none cursor-pointer"
          >
            Discard
          </button>
          <div className="flex gap-3">
            <button
              ...
              className="px-6 py-4 rounded-full text-sm font-bold bg-[#373b53] hover:bg-[#0c0e16] text-[#888eb0] transition-colors border-none cursor-pointer"
            >
              Save as Draft
            </button>
            <button
              onClick={() => handleSave("Pending")}
              className="px-6 py-4 rounded-full text-sm font-bold bg-[#7c5dfa] hover:bg-[#9277ff] text-white transition-colors border-none cursor-pointer"
            >
              Save &amp; Send
            </button>
          </div>
